...
*/
  ...
  addr.sin_port = htons(sockNum);
  addr.sin_addr.s_addr = INADDR_ANY;
  memset(addr.sin_zero, '\0', sizeof(addr.sin_zero));

  jp_comm_m = socket(PF_INET, SOCK_STREAM, 0);

  vpi_printf("port num %u!\n", (uint16_t)atoi(value.value.str));
#ifdef WIN32
